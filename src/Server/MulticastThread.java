	...
	public void run() {
		try {
			...
			InetAddress inetaddr = InetAddress
					.getByName("experiment.mcast.net");
			ms.joinGroup(inetaddr);
			while (isAlive) {
				byte[] receive = new byte[100];
				DatagramPacket dprec = new DatagramPacket(receive,
						receive.length);
				ms.receive(dprec);
				...
				if (s.equals("quickz")) {
					...
					DatagramPacket dpsend = new DatagramPacket(send,
							send.length, dprec.getAddress(), dprec.getPort());
					ms.send(dpsend);
				}
			}
		} catch (Exception e) {
